...
import { createRepairPaymentSchema } from "@/server/validation/repairOperations.validation";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = parseRequestPayload(createRepairPaymentSchema, await request.json(), REPAIR_TEXT.invalidRequest);
    return NextResponse.json(await RepairService.addPayment(identity, id, payload), {
