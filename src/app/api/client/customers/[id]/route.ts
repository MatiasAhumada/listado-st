...
import { updateWorkshopCustomerSchema } from "@/server/validation/workshopOperations.validation";
...
export async function PUT(
  ...
  try {
    ...
    const payload = parseRequestPayload(
      updateWorkshopCustomerSchema,
      await request.json(),
      TECHNICIAN_TEXT.invalidRequest
    );
    return NextResponse.json(
      await WorkshopCustomerService.updateCustomer(identity, id, payload)
    );
  } catch (error) {
